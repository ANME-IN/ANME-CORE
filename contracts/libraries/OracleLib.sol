...
library OracleLib {
     ...
     */
    function _staleCheckLatestRoundData(AggregatorV3Interface _chainlinkFeed)
        internal
        view
     ...
     *
     */
    function getUsdValue(AggregatorV3Interface _priceFeed, uint256 _amount) internal view returns (uint256) {
        (, int256 price,,,) = _staleCheckLatestRoundData(_priceFeed);
        // 1 ETH = 1000 USD
    ...
    function getEthAmountFromUsd(AggregatorV3Interface _priceFeed, uint256 _usdAmount)
        ...
        returns (uint256)
    {
        (, int256 ethPrice,,,) = _staleCheckLatestRoundData(_priceFeed);
